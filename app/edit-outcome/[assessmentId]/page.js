  ...
  return (
    <div className="card">
      ...
      <form onSubmit={handleSubmit} autoComplete="off">
        ...
        <PrescriptionFields defaults={{
          medicine_prescribed: assessment.medicine_prescribed,
          potency: assessment.potency,
          dose_doses: assessment.dose_doses
        }} />
        ...
        {assessment.assessment_number === 6 && (
          <>
            ...
            {miasm_questions.map((q, i) => (
              <div key={`m${i}`}>
                <label>{i + 1}. {q}:</label>
                <input type="text" name={`miasm_${i + 1}`} defaultValue={md[`miasm_${i + 1}`] || '0'} />
              </div>
            ))}
