...
// We use server-side env vars here (no NEXT_PUBLIC_ prefix needed, but we reuse the same ones)
const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
)

// --- Miasm scoring logic (duplicated server-side for export) ---
...
export async function GET() {
  ...
  patients.forEach(p => ws1.addRow(p))

  // --- Sheet 2: Socio-demographic ---
  ...
  patients.forEach(p => ws2.addRow(p))

  // --- Sheet 3: Baseline Treatment ---
  ...
  const blCols = [
    ...
    { header: 'Prescription', key: 'prescription', width: 25 },
  ]
  ...
  patients.forEach(p => {
    ...
    const row = { reg_no: p.reg_no, name: p.name, bl_date: bl.date, present_complaint: bl.present_complaint, prescription: bl.prescription }
    miasm_questions.forEach((_, i) => { row[`mq_${i+1}`] = mj[`miasm_${i+1}`] || '' })
  ...
  for (let num = 1; num <= 6; num++) {
    ...
    patients.forEach(p => {
      ...
      const row = {
        ...
        brief_notes: oa.brief_notes, prescription: oa.prescription,
    ...
    if (num === 6) {
      ws.getRow(1).eachCell((cell) => {
        ...
        else if (v.startsWith('ORIDL')) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFFFE0' } }
      })
    }
  }
